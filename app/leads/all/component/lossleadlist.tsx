...
interface LossLeadListProps {
  searchQuery: string;
  setdata: any;
}

export default function Lossleadlist({
  searchQuery,
  setdata,
}: LossLeadListProps) {
  const [lossLeads, setlossLeads] = useState<Lead[]>([]);
  ...
  const fetchAllLead = async () => {
    ...
    try {
      ...
      }>(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/leads/lost`, {
        params: {
          limit: pageLimit,
          page: currentPage,
        },
      ...
      setlossLeads(response.data.leads);
    ...
    <div className="flex-1 space-y-4">
      <div className="max-sm:hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64">
            <p className="text-red-500 text-lg">Failed to load loss leads.</p>
      ...
      <div className="sm:hidden space-y-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64">
            <p className="text-red-500 text-lg">Failed to load lost leads.</p>
            <Button onClick={handleRetry} className="mt-4">
              Retry
            </Button>
          </div>
